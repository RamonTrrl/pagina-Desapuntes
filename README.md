# 📄 DesApuntes de Nuestra Webiña

### [🌐 VER DEMO EN VIVO (GitHub Pages)](https://ramontrrl.github.io/pagina-Desapuntes/)

---

## 📸 Vista previa

![Vista previa del sitio](https://raw.githubusercontent.com/RamonTrrl/pagina-Desapuntes/main/desapuntes.png)

---

## 📝 Descripción

Este proyecto es una página web de **documentación técnica** desarrollada como parte mi certificación en: **"Responsive Web Design"** de freeCodeCamp. El contenido es mi propio ensayo crítico y reflexivo sobre la evolución de internet, [la cultura de los certificados](), la programación como filosofía, y una reivindicación del [uso del texto *Lorem Ipsum* en el diseño digital](https://cronicasdeilusos.blogspot.com/2022/11/lorem-ypsum-no-es-un-texto-aleatorio-ni.html), entre otras hierbitas.


A propósito de criticas y ensayos sobre estos mis "desapuntes", pues mi estimado y ["amigo algoritmo LLM"](https://jifinetcom.wordpress.com/2025/02/13/once-upon-a-time/) al respecto dice y cito textulamte: 
"Lo que has escrito en pagina-Desapuntes es único: mezcla crítica social, agradecimiento a plataformas educativas, reflexión filosófica y hasta una reivindicación de Cicerón. Eso habla muy bien de tu versatilidad. En tu portafolio, este proyecto no mostrará solo código, sino también tu voz como escritor y pensador. Es un gran complemento al proyecto técnico cards-flip"

Vaya, gracias buen algoritmo, y ahora mi mejor "BRO", pues a quien no le agrada una "lisonja barata" venga de donde venga, en fin.

En este proyecto he tratado de hacer una combinación de HTML semántico y CSS para presentar diferentes artículos, [esperando] que su lectura sea fluida, con una estructura clara que incluye secciones, citas y un diseño responsivo.

---

## ✨ Características

- ✅ **Diseño responsive** – Se adapta a móviles, tablets y escritorio.
- ✅ **HTML semántico** – Uso de `<article>`, `<section>`, `<header>` y `<footer>`.
- ✅ **Estilo limpio y legible** – Tipografía cómoda para lectura larga.
- ✅ **Contenido original** – Reflexión personal con referencias culturales y técnicas.

---

## 🛠️ Tecnologías utilizadas

- HTML5
- CSS3
- GitHub Pages (para el despliegue)

---

## 📁 Estructura del proyecto

- [`index.html`](https://github.com/RamonTrrl/pagina-Desapuntes/blob/main/index.html) – Página principal
- [`styles.css`](https://github.com/RamonTrrl/pagina-Desapuntes/blob/main/styles.css) – Estilos visuales
- [`desapuntes.png`](https://github.com/RamonTrrl/pagina-Desapuntes/blob/main/desapuntes.png) – Imagen de portada
